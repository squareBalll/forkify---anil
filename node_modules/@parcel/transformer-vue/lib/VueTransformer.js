...
var _default = new (_plugin().Transformer)({
  ...
  async parse({
    ...
    let compiler = await options.packageManager.require('@vue/compiler-sfc', asset.filePath, {
  ...
  async transform({
    ...
    let baseId = (0, _utils().md5FromObject)({
...
async function processPipeline({
  ...
  let consolidate = await options.packageManager.require('consolidate', asset.filePath, {
    autoinstall: options.autoinstall
  });
    ...
    case 'custom':
      {
        ...
        return [{
          ...
          content: `
let NOOP = () => {};
${(await Promise.all([...types].map(async (type) => `import p${type} from './${(0, _path().relative)((0, _path().dirname)(asset.filePath), await resolve(config.filePath, config.customBlocks[type]))}';

...
var _default = new (_plugin().Transformer)({
  ...
  async transform({
    ...
    let stylusConfig = config ? config.contents : {}; // stylus should be installed locally in the module that's being required
    ...
    style.set('Evaluator', await createEvaluator(code, asset, resolve, style.options, options));
    asset.type = 'css';
...
async function getDependencies(code, filepath, asset, resolve, options, parcelOptions, seen = new Set()) {
  seen.add(filepath);
  const [Parser, DepsResolver, nodes, utils] = await Promise.all(['parser', 'visitor/deps-resolver', 'nodes', 'utils'].map(dep => parcelOptions.packageManager.require('stylus/lib/' + dep, filepath, {
    autoinstall: options.autoinstall
  })));
  ...
  let deps = new Map();
  new class ImportVisitor extends DepsResolver {
    visitImport(imported) {
      let importedPath = imported.path.first.string;

      if (!deps.has(importedPath)) {
        if ((0, _utils().isGlob)(importedPath)) {
          deps.set(importedPath, (0, _utils().glob)(_path().default.resolve(_path().default.dirname(filepath), importedPath), parcelOptions.inputFS, {
            onlyFiles: true
  ...
  }(ast, options).visit(ast); // Recursively process depdendencies, and return a map with all resolved paths.
  ...
  await Promise.all(Array.from(deps.entries()).map(async ([importedPath, resolved]) => {
    ...
    if (resolved) {
      ...
      let paths = (options.paths || []).concat(_path().default.dirname(filepath || '.'));
    ...
    for (let resolved of found) {
      if (!seen.has(resolved)) {
        ...
        for (let [path, resolvedPath] of await getDependencies(code, resolved, asset, resolve, options, parcelOptions, seen)) {
...
async function createEvaluator(code, asset, resolve, options, parcelOptions) {
  const deps = await getDependencies(code, asset.filePath, asset, resolve, options, parcelOptions);
  const Evaluator = await parcelOptions.packageManager.require('stylus/lib/visitor/evaluator', asset.filePath); // This is a custom stylus evaluator that extends stylus with support for the node

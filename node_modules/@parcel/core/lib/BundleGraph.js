...
class BundleGraph {
  ...
  removeAssetGraphFromBundle(asset, bundle) {
    ...
    this._graph.traverse((node, context, actions) => {
      ...
      if (node.type === 'asset' || node.type === 'dependency') {
        if (this._graph.hasEdge(bundle.id, node.id, 'contains') && (node.type !== 'asset' || this.isAssetReachableFromBundle(node.value, bundle))) {
  ...
  removeBundle(bundle) {
    ...
    this._graph.removeNode(bundleNode); // Remove bundle group node if it no longer has any bundles
  ...
  getBundlesInBundleGroup(bundleGroup) {
    return bundleGroup.bundleIds.map(id => {
      let b = (0, _nullthrows().default)(this._graph.getNode(id));
      (0, _assert().default)(b.type === 'bundle');
      return b.value;
    }).reverse();
  }
  ...
  getReferencedBundles(bundle) {
    return this._graph.getNodesConnectedFrom((0, _nullthrows().default)(this._graph.getNode(bundle.id)), 'references').filter(node => node.type === 'bundle').map(node => {
      (0, _assert().default)(node.type === 'bundle');
      return node.value;
    });
  }
  ...
  resolveSymbol(asset, symbol, boundary) {
    ...
    if (potentialResults.length == 1) {
      ...
      return {
        asset,
        exportSymbol: symbol,
        symbol: skipped ? false : found ? null : identifier !== null && identifier !== void 0 ? identifier : ((_asset$symbols5 = asset.symbols) === null || _asset$symbols5 === void 0 ? void 0 : _asset$symbols5.has('*')) ? null : undefined,

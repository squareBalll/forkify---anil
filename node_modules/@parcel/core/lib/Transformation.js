...
class Transformation {
  ...
  async runPipelines(pipeline, initialAsset) {
    ...
    if (!initialCacheEntry) {
      let resultCacheKey = this.getCacheKey([initialAsset], pipeline.configs, await (0, _assetUtils().getInvalidationHash)((0, _utils().flatMap)(assets, asset => asset.getInvalidations()), this.options));
    ...
    let processedCacheEntry = await this.readFromCache(this.getCacheKey(finalAssets, pipeline.configs, await (0, _assetUtils().getInvalidationHash)((0, _utils().flatMap)(finalAssets, asset => asset.getInvalidations()), this.options)));
    ...
    if (!processedCacheEntry) {
      await this.writeToCache(this.getCacheKey(processedFinalAssets, pipeline.configs, await (0, _assetUtils().getInvalidationHash)((0, _utils().flatMap)(processedFinalAssets, asset => asset.getInvalidations()), this.options)), processedFinalAssets, pipeline.configs);
    }
  ...
  async runPipeline(pipeline, initialAsset) {
    ...
    for (let transformer of pipeline.transformers) {
      ...
      for (let asset of inputAssets) {
        ...
        try {
          let transformerResults = await runTransformer(pipeline, asset, transformer.plugin, transformer.name, transformer.config, transformer.configKeyPath, this.parcelConfig);
  ...
  async writeToCache(cacheKey, assets, configs) {
    await Promise.all(assets.map(asset => asset.commit((0, _utils().md5FromObject)({
...
async function runTransformer(pipeline, asset, transformer, transformerName, preloadedConfig, configKeyPath, parcelConfig) {
  ...
  if (postProcess) {
    pipeline.postProcess = async (assets) => {
      ...
      return Promise.all(results.map(result => asset.createChildAsset(result, transformerName, parcelConfig.filePath, configKeyPath)));
    };
  }

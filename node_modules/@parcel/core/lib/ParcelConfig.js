...
class ParcelConfig {
  constructor(config, packageManager, fs, autoinstall) {
  ...
  static deserialize(serialized) {
    return new ParcelConfig(serialized.config, serialized.packageManager, serialized.fs, serialized.autoinstall);
  }
  ...
  loadPlugin(node) {
    ...
    plugin = (0, _loadParcelPlugin().default)(this.fs, this.packageManager, node.packageName, node.resolveFrom, node.keyPath, this.autoinstall);
  ...
  isGlobMatch(filePath, pattern, pipeline) {
    ...
    return (pipeline === patternPipeline || !pipeline && !patternPipeline) && ((0, _micromatch().isMatch)(filePath, patternGlob) || (0, _micromatch().isMatch)((0, _path().basename)(filePath), patternGlob));
  }

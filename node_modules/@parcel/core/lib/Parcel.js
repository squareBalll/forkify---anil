...
class Parcel {
  ...
  async _init() {
    ...
    if (_classPrivateFieldGet(this, _initialOptions).workerFarm) {
      ...
      // Parcel ends.
      _classPrivateFieldGet(this, _disposable).add(disposeOptions, disposeConfig);
    } else {
  ...
  async _end() {
    ...
    await Promise.all([_classPrivateFieldGet(this, _disposable).dispose(), _classPrivateFieldGet(this, _assetGraphBuilder).writeToCache(), _classPrivateFieldGet(this, _runtimesAssetGraphBuilder).writeToCache()]);
  }
  ...
  async _build({
    ...
    try {
      ...
      let {
        assetGraph,
        changedAssets
      } = await _classPrivateFieldGet(this, _assetGraphBuilder).build(signal); // $FlowFixMe Added in Flow 0.121.0 upgrade in #4381

      let [bundleGraph, serializedBundleGraph] = await _classPrivateFieldGet(this, _bundlerRunner).bundle(assetGraph, {
        signal
      }); // $FlowFixMe Added in Flow 0.121.0 upgrade in #4381 (Windows only)

      (0, _dumpGraphToGraphViz().default)(bundleGraph._graph, 'BundleGraph');
      await _classPrivateFieldGet(this, _packagerRunner).writeBundles(bundleGraph, serializedBundleGraph);

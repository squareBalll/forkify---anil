...
function _serverErrors() {
  const data = _interopRequireDefault(require("./serverErrors"));

  _serverErrors = function () {
    return data;
  };
...
class Server extends _events().default {
  constructor(options) {
    ...
    _defineProperty(this, "stopServer", void 0);
  ...
  sendIndex(req, res) {
    if (this.bundleGraph) {
      ...
      let htmlBundle = this.bundleGraph.traverseBundles((bundle, context, {
        stop
      }) => {
        ...
        if (context && bundle.filePath && bundle.filePath.endsWith('index.html')) {
          stop();
          return bundle;
        }
      });
      ...
      if (htmlBundle) {
        req.url = `/${_path().default.relative(this.options.distDir, (0, _nullthrows().default)(htmlBundle.filePath))}`;
  ...
  async applyProxyTable(app) {
    ...
    const pkg = await (0, _utils().loadConfig)(this.options.inputFS, fileInRoot, ['.proxyrc.js', '.proxyrc', '.proxyrc.json']);

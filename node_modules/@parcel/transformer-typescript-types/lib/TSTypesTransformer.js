...
function _TSModuleGraph() {
  const data = require("./TSModuleGraph");

  _TSModuleGraph = function () {
    return data;
  };
...
var _default = new (_plugin().Transformer)({
  ...
  async transform({
    ...
    let host = new (_tsUtils().CompilerHost)(options.inputFS, ts, logger); // $FlowFixMe
    ...
    let includedFiles = program.getSourceFiles().filter(file => _path().default.normalize(file.fileName) !== asset.filePath).map(file => {
      ...
      return {
        filePath: (_host$redirectTypes$g = host.redirectTypes.get(file.fileName)) !== null && _host$redirectTypes$g !== void 0 ? _host$redirectTypes$g : file.fileName
      };
    });

...
require('v8-compile-cache'); // Exit codes in response to signals are traditionally
// 128 + signal value
...
async function run(entries, command) {
  ...
  let packageManager = new (_packageManager().NodePackageManager)(fs);
    ...
    let stream = process.stdin.on('keypress', async (char, key) => {
      ...
      switch (key.name) {
        case 'c':
          ...
          // within the timeout.
          setTimeout(() => _logger().INTERNAL_ORIGINAL_CONSOLE.log(_chalk().default.bold.yellowBright('Parcel is shutting down...')), 500); // When watching, a 0 success code is acceptable when Parcel is interrupted with ctrl-c.
